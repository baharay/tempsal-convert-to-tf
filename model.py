...
class PNASModel(nn.Module):
    ...
    def forward(self, images):
        ...
        # Print shape of out5
        print(f"Shape after deconv_layer0 (out5): {out5.shape}")

        x = torch.cat((out5, out4), 1)
        # Print shape after concatenation of out5 and out4
        print(f"Shape after concatenating out5 and out4: {x.shape}")

        x = self.deconv_layer1(x)
        # Print shape after deconv_layer1
        print(f"Shape after deconv_layer1: {x.shape}")

        x = torch.cat((x, out3), 1)
        # Print shape after concatenating x and out3
        print(f"Shape after concatenating x and out3: {x.shape}")

        x = self.deconv_layer2(x)
        # Print shape after deconv_layer2
        print(f"Shape after deconv_layer2: {x.shape}")

        x = torch.cat((x, out2), 1)
        # Print shape after concatenating x and out2
        print(f"Shape after concatenating x and out2: {x.shape}")

        x = self.deconv_layer3(x)
        # Print shape after deconv_layer3
        print(f"Shape after deconv_layer3: {x.shape}")

        x = torch.cat((x, out1), 1)
        # Print shape after concatenating x and out1
        print(f"Shape after concatenating x and out1: {x.shape}")

        x = self.deconv_layer4(x)
        # Print shape after deconv_layer4
        print(f"Shape after deconv_layer4: {x.shape}")

        x = self.deconv_layer5(x)
        # Print shape after deconv_layer5
        print(f"Shape after deconv_layer5: {x.shape}")

        x = x.squeeze(1)
        # Print final shape after squeezing
        print(f"Final shape after squeeze: {x.shape}")
